...
export class KiteEntityFactory {
  static create(controlBarPosition: THREE.Vector3): Entity {
    ...
    // ✨ MAKANI-INSPIRED: Orientation initiale face au vent avec angle d'attaque optimal
    // Utilise la configuration centralisée pour cohérence
    const { pitchDeg, yawDeg, rollDeg } = CONFIG.initialization.initialKiteOrientation;
    const initialOrientation = MathUtils.createInitialKiteOrientation(pitchDeg, yawDeg, rollDeg);

    // Composant Transform avec orientation initiale
    kiteEntity.addComponent(new TransformComponent({
    ...
    // ✅ NOUVEAU: Points de contrôle CTRL comme points locaux du kite
    // Architecture couplée inspirée de la branche main + Position-Based Dynamics
    // Les CTRL suivent le kite (coordonnées locales solidaires)
    // Position: Légèrement en bas et arrière du centre, écartés latéralement
    geometry.setPoint('CTRL_GAUCHE', KiteGeometry.POINTS.CTRL_GAUCHE.clone());
    geometry.setPoint('CTRL_DROIT', KiteGeometry.POINTS.CTRL_DROIT.clone());
