...
export class KiteGeometry {
  ...
  /**
   * Points anatomiques du cerf-volant
   *
   * NOUVEAUTÉ: Ajout des points CTRL_GAUCHE/CTRL_DROIT (points de contrôle)
   * Ces points sont les positions d'attache des lignes de contrôle sur le kite.
   * Ils étaient auparavant calculés dynamiquement, mais sont maintenant définis
   * comme points locaux du kite (architecture couplée inspirée branche main).
   *
   * Position CTRL: Légèrement derrière et en bas du centre, pour permettre
   * le contrôle directionnel via les brides.
   */
  static readonly POINTS = {
    ...
    // ✅ NOUVEAU: Points de contrôle pour les lignes
    // Position: En bas et légèrement arrière du centre, écartés latéralement
    // Ces points SUIVENT le kite (coordonnées locales solidaires)
    CTRL_GAUCHE: new THREE.Vector3(-0.3, -0.4, 0.2), // Point contrôle gauche
    CTRL_DROIT: new THREE.Vector3(0.3, -0.4, 0.2), // Point contrôle droit

    // Points d'attache des brides (pour référence)
    INTER_GAUCHE: new THREE.Vector3(-0.25, 0.3, 0), // Attache bride intermédiaire gauche
    INTER_DROIT: new THREE.Vector3(0.25, 0.3, 0), // Attache bride intermédiaire droite
    CENTRE: new THREE.Vector3(0, 0, 0), // Attache bride centrale
  };
  ...
  static calculateInertia(): number {
    ...
    // ✨ MAKANI-INSPIRED: Utiliser l'inertie physique réelle (DIAGNOSTIC_VOL.md Problème #6)
    // ANCIEN: Factor 0.1 rendait le kite trop réactif (0.042 kg·m²)
    // NOUVEAU: Inertie physique complète (~0.42 kg·m²) pour comportement réaliste
