...
export class RenderManager {
  ...
  private setupEnvironment(): void {
    ...
    const groundMaterial = new THREE.MeshLambertMaterial({ color: 0x228b22 }); // Vert forêt
  ...
  private addClouds(): void {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        ...
        cloudPart.position.x = Math.random() * 15 - 7.5;
        cloudPart.position.y = Math.random() * 3 - 1.5;
        cloudPart.position.z = Math.random() * 15 - 7.5;
        cloudPart.scale.setScalar(Math.random() * 0.8 + 0.6);
      ...
      cloud.position.set(
        (Math.random() - 0.5) * 400, // X: -200 à 200
        Math.random() * 40 + 25, // Y: 25 à 65 (hauteur dans le ciel)
        (Math.random() - 0.5) * 400 // Z: -200 à 200
      );
