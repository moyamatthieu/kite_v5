...
export class Kite extends StructuredObject implements ICreatable {
  ...
  protected buildStructure(): void {
    ...
    const mainFrameParams = {
      ...
      material: frameColor,
      points: Array.from(this.pointsMap.entries()), // Points comme Point class
      connections: [
        // Épine centrale
  ...
  private createBridleLines(): void {
    ...
    bridleConnections.forEach(([startName, endName]) => {
      const startPoint = this.pointsMap.get(startName);
      const endPoint = this.pointsMap.get(endName);

      if (startPoint && endPoint) {
        // Créer une ligne droite simple (sans effet caténaire pour les brides internes)
        const geometry = new THREE.BufferGeometry();
        const points = [
          startPoint.toVector3(),
          endPoint.toVector3(),
        ];
        ...
        const restLength = startPoint.distanceTo(endPoint);
   ...
   */
  public getPointsMap(): Map<string, Point> {
  ...
  private addVisualMarkers(): void {
    // Nez (point rouge)
    const nezPoint = this.pointsMap.get("NEZ");
    if (nezPoint) {
      const nez = nezPoint.toVector3();
      const marker = Primitive.sphere(0.025, "#ff0000");
      this.addPrimitiveAt(marker, [nez.x, nez.y, nez.z]);
    }

    // Points de contrôle
    const ctrlGPoint = this.pointsMap.get("CTRL_GAUCHE");
    if (ctrlGPoint) {
      const ctrlG = ctrlGPoint.toVector3();
      const marker = Primitive.sphere(0.025, "#dc143c");
      this.addPrimitiveAt(marker, [ctrlG.x, ctrlG.y, ctrlG.z]);
    }

    const ctrlDPoint = this.pointsMap.get("CTRL_DROIT");
    if (ctrlDPoint) {
      const ctrlD = ctrlDPoint.toVector3();
